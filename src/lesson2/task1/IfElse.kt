...
fun ageDescription(age: Int): String = if (age % 100 < 10 || age % 100 > 20) {
    when (age % 10) {
        1 -> "$age год"
        in 2..4 -> "$age года"
        else -> "$age лет"
 ...
 */
fun triangleKind(a: Double, b: Double, c: Double): Int = if (a < b + c && b < a + c && c < a + b) {
    val max = maxOf(a, b, c)
    val min = minOf(a, b, c)
    val t = a + b + c - max - min
    when {
        max * max > min * min + t * t -> 2
        max * max < min * min + t * t -> 0
        else -> 1
    }
} else -1
